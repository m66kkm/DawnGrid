...
#[test]
fn test_read_range_clamps_out_of_bounds() {
    let path = std::path::PathBuf::from(r"A:\Download\研发中心代码By仓库统计分析.xlsx");
    if !path.exists() {
        return;
    }
    let mut sessions = WorkbookSessions::new();
    let meta = sessions.open_with_locale(&path, "zh", None).unwrap();
    let s0 = &meta.sheets[0];
    // Request range deliberately outside bounds (s0 has col_count=20, so col 50 is outside)
    let range = CellRange {
        start_row: 0,
        end_row: 100,
        start_column: 0,
    ...
    let res = sessions.read_range(&meta.session_id, &s0.id, &range).unwrap();
