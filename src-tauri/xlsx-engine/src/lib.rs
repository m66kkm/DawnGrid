...
impl WorkbookSessions {
    ...
    pub fn read_range(
        ...
        let sheet = &session.sheets[sheet_index];
        ...
        if range.start_row >= sheet.row_count || range.start_column >= sheet.column_count {
            return Ok(RangeResult::empty());
        }
        let mut clamped = range.clone();
        if clamped.end_row >= sheet.row_count {
            clamped.end_row = sheet.row_count - 1;
        }
        if clamped.end_column >= sheet.column_count {
            clamped.end_column = sheet.column_count - 1;
        }
        clamped.validate(sheet)?;
        session.ensure_parser(sheet_index)?;
        session.read_range(sheet_index, &clamped)
    }
